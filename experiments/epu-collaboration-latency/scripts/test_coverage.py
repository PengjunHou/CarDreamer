"""Offline tests for the ported coverage-uncertainty module (no CARLA, no torch).

Ported from Coopeception-V4 tests/test_wam_coverage.py; only the import is adapted to the local
coverage.py (sys.path inserted first so our module wins over any pip `coverage` package).
"""
import os
import sys
import unittest
...
from coverage import BevSpec, CoverageConfig, build_coverage_raster, coverage_metrics  # noqa: E402
...
class CoverageRasterTest(unittest.TestCase):
    ...
    def test_stale_collaborator_improves_coverage_less(self):
        ...
        # coverage_uncertainty monotone in freshness: fresh < stale < ego_only
