...
STEPS=1000
MIN_FRAMES=600
GEOM=./logdir/group_epu_geom

ensure_carla() {
    if ! pgrep -f "CarlaUE4-Linux.*-carla-port=$PORT" > /dev/null; then
        echo "[ecpg] launching CARLA..."
        nohup "$CARLA_ROOT/CarlaUE4.sh" -RenderOffScreen -carla-port=$PORT -benchmark -fps=10 > carla_group_epu.log 2>&1 &
...
run_cfg() {
    ...
    for attempt in 1 2 3 4; do
        ...
        PYTHONPATH="$(pwd)" conda run -n cardreamer python -X faulthandler -u dreamerv3/eval.py \
            ...
            --dreamerv3.run.from_checkpoint "$CKPT" --task carla_group_right_turn_auto \
