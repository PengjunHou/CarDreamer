# coop_sac_agent.py  (CONTINUOUS ACTION VERSION, signatures unchanged)
...
# -------------------------
# Continuous SAC modules
# -------------------------
class ContinuousActor(nn.Module):
    """
    Gaussian policy with tanh-squash, then affine scale to env bounds.
    """
    def __init__(
        self,
        enc_dim: int,
        ...
        # bounds: shape [act_dim]
        self.register_buffer("act_low", act_low.clone())
        self.register_buffer("act_high", act_high.clone())
    ...
    def _squash_and_scale(self, u: torch.Tensor) -> torch.Tensor:
        # u in [-1, 1] -> action in [low, high]
        return u * self.act_scale + self.act_bias

    def sample(self, h: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Return:
          action: [B, act_dim] (scaled to env bounds)
          logp:   [B, 1] log prob of scaled action
        """
        ...
        u = torch.tanh(z)                     # [-1,1]
        a = self._squash_and_scale(u)         # [low,high]

        # log prob with tanh correction + affine scale correction
        # logp_u = sum_i (log N(z_i|mu,std) - log(1 - tanh(z_i)^2))
        logp = dist.log_prob(z) - torch.log(1.0 - u.pow(2) + self.eps)
        logp = logp.sum(dim=-1, keepdim=True)

        # affine scale: a = u * scale + bias -> |da/du| = scale (per-dim)
        # so logp_a = logp_u - sum log(scale)
        logp = logp - torch.log(self.act_scale + self.eps).sum(dim=-1, keepdim=True)
...
class ContinuousCritic(nn.Module):
    ...
    Q(h, a) -> scalar
    """
    def __init__(self, enc_dim: int, act_dim: int, hidden: int = 256):
        super().__init__()
...
class CoopSACConfig:
    ...
    # continuous SAC 常用 target entropy
    # 你也可以改成 -(act_dim) 或者 -(act_dim)*some_factor
...
class CoopSACAgent(embodied.Agent):
    def __init__(self, obs_space, act_space, step, config: CoopSACConfig):
        ...
        # ---- Detect action space type (dict or Box)
        ...
        if self._act_is_dict:
            ...
            self.act_dim = int(shape[0])

            # try read bounds (Box)
            low = getattr(sub, "low", None)
            high = getattr(sub, "high", None)
            if low is None or high is None:
                raise ValueError("Continuous SAC requires Box(low, high) for dict action space sub-space.")
            self.act_low = torch.as_tensor(np.asarray(low, np.float32), device=self.device)
            self.act_high = torch.as_tensor(np.asarray(high, np.float32), device=self.device)

            self._output_dict = True
            print(f"[CoopSACAgent] Detected dict Box action space '{self.cfg.action_key}', act_dim={self.act_dim}")
        else:
            # gymnasium Box
            shape = getattr(act_space, "shape", None)
            if not shape or len(shape) != 1 or int(shape[0]) <= 0:
                raise ValueError(f"Continuous SAC requires Box(shape=(A,)), got {act_space}")
            self.act_dim = int(shape[0])

            low = getattr(act_space, "low", None)
            high = getattr(act_space, "high", None)
            if low is None or high is None:
                raise ValueError("Continuous SAC requires Box(low, high).")
            self.act_low = torch.as_tensor(np.asarray(low, np.float32), device=self.device)
            self.act_high = torch.as_tensor(np.asarray(high, np.float32), device=self.device)

            self._output_dict = False

        # target entropy: usually -act_dim
        self.target_entropy = torch.tensor(
            -float(self.act_dim) * float(self.cfg.target_entropy_scale),
            device=self.device
        )
        ...
        self.q1 = ContinuousCritic(enc_dim, act_dim=self.act_dim).to(self.device)
        self.q2 = ContinuousCritic(enc_dim, act_dim=self.act_dim).to(self.device)
        self.tq1 = ContinuousCritic(enc_dim, act_dim=self.act_dim).to(self.device)
        self.tq2 = ContinuousCritic(enc_dim, act_dim=self.act_dim).to(self.device)
    ...
    def policy(self, obs: Dict[str, Any], state=None, mode="train"):
        """
        连续动作 SAC:
        - eval: deterministic (mode)
        - train/explore: stochastic sample
        """
        self.encoder.eval()
        self.actor.eval()
        ...
        for b in range(B):
            ...
            if mode == "eval":
                a = self.actor.mode(h)          # [1,act_dim]
            else:
                a, _ = self.actor.sample(h)     # [1,act_dim]

            acts.append(a)

        a_out = torch.cat(acts, dim=0).cpu().numpy().astype(np.float32)  # [B, act_dim]
    ...
    def train(self, data: Dict[str, Any], state=None):
        ...
        def _ensure_BL_from_data(d: Dict[str, Any]) -> Dict[str, Any]:
            ...
            # action: expect float [..., act_dim]
            act = _as_np(out[self.cfg.action_key])
            if no_batch:
                # allow [L,act_dim] -> [1,L,act_dim]
                if act.ndim == 2 and act.shape[-1] == self.act_dim:
                    act = act[None, ...]
                else:
                    raise ValueError(f"Expected action [L,{self.act_dim}] when no batch, got {act.shape}")
            else:
                # expect [B,L,act_dim]
                if not (act.ndim == 3 and act.shape[-1] == self.act_dim):
                    raise ValueError(f"Expected action [B,L,{self.act_dim}], got {act.shape}")
            out[self.cfg.action_key] = act.astype(np.float32)
        ...
        action = _as_np(seq[self.cfg.action_key]).astype(np.float32)  # [B,L,act_dim]
        ...
        for b in range(B):
            ...
            hs.append(h)
            nhs.append(nh)
            a_t.append(action[b, t])      # [act_dim]
            r_t.append(r_seq[b, t])
            disc_t.append([disc])

        h = torch.stack(hs, dim=0).to(self.device)   # [B,H]
        nh = torch.stack(nhs, dim=0).to(self.device) # [B,H]
        a = _to_torch(np.asarray(a_t, np.float32), self.device).float().view(B, self.act_dim)  # [B,act_dim]
        ...
        # ---- Critic target
        with torch.no_grad():
            next_a, next_logp = self.actor.sample(nh)            # [B,act_dim], [B,1]
            tmin = torch.min(self.tq1(nh, next_a), self.tq2(nh, next_a))  # [B,1]
            alpha = self.log_alpha.exp()
            target_q = r + disc * self.cfg.gamma * (tmin - alpha * next_logp)  # [B,1]
        ...
        # IMPORTANT: use h.detach() so actor update doesn't touch encoder
        new_a, logp = self.actor.sample(h.detach())
        min_q = torch.min(self.q1(h.detach(), new_a), self.q2(h.detach(), new_a))
        alpha = self.log_alpha.exp()
        ...
        if self.autotune:
            # standard: minimize E[ alpha * (-logp - target_entropy) ]
    ...
    def save(self):
        return {
            ...
            "output_dict": bool(self._output_dict),
        }
    ...
    def load(self, data):
        ...
        if "act_dim" in data:
            self.act_dim = int(data["act_dim"])
            self.target_entropy = torch.tensor(
                -float(self.act_dim) * float(self.cfg.target_entropy_scale),
                device=self.device
            )
        if "act_low" in data and "act_high" in data:
            self.act_low = torch.as_tensor(np.asarray(data["act_low"], np.float32), device=self.device)
            self.act_high = torch.as_tensor(np.asarray(data["act_high"], np.float32), device=self.device)
        if "output_dict" in data:
            self._output_dict = bool(data["output_dict"])
