...
3. **policy-augmented recorder**([stage1_policy.py](car_dreamer/toolkit/wam/stage1_policy.py))为单图 per-policy 评估,
   非时序窗口,本次未改;其无 source state 时 `perception_labels=None` → 训练回退节点标签。
